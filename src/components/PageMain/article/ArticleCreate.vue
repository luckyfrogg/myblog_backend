        ...
        <a-row type="flex" align="middle" class="form_wrap">
            ...
            <a-col>
                <div class="cate_btn_group">
                    <a-button type="default" v-for="(item,index) in cateArr" :key="index" size="small" :class="['cate_btn',item.actived?'active':'']" @click="checkCate(index)">{{item.name}}</a-button>
                    <a-button type="primary" size="small" :class="['cate_btn']" @click="showCreateCatePop">+</a-button>
                </div>
            </a-col>
        </a-row>
    </div>
...
export default {
  ...
  methods: {
      ...
      createCate(){
          let _this=this;
          this.$post("api/b/createCate",{name:_this.createCateVal.trim()}).then((res)=>{
              if(res)
